...
By default, you can set a users workspace by passing their workspace details to `CollabKitProvider`.

{renderCodeSnippet(Usage)}

### Usage (with Secure Mode)

If you have [Secure Mode](/docs/secure-mode) enabled you will need to generate a per-user token for each user in your app and pass this to `CollabKitProvider` instead of the workspace details.
