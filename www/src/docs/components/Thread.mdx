...
import { ThemeBase } from '@collabkit/react';
...
The `Thread` component is composed of the following components:
...
You can use these components to build your own custom `Thread` component. `CommentList` and `Composer` are also composable, so you can split them apart to

```tsx
import { ThreadRoot, Scrollable, CommentList, Composer } from '@collakbit/react';

function CustomThread(props: { threadId: string }) {
  return (
...
To style indivdual components, adjust your theme or CSS variables. Or provide a `className` prop to a leaf component.

For example, if you want to customise the `ThreadRoot` component you can do so by passing in a `className` prop:

```tsx
<ThreadRoot threadId={threadId} className="your-thread-classname-here">
```
