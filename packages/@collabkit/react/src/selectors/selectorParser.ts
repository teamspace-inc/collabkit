...
export function parseSelector(selector: string): ParsedSelector {
  ...
  const parts: ParsedSelectorPart[] = result.parts.map((part) => {
    ...
    if (kNestedSelectorNames.has(part.name)) {
      ...
      if (
        result.body.parsed.parts.some(
          (part) => part.name === 'control' && part.body === 'enter-frame'
        )
      )
...
export function allEngineNames(selector: ParsedSelector): Set<string> {
  ...
  const visit = (selector: ParsedSelector) => {
    for (const part of selector.parts) {
      ...
      if (kNestedSelectorNames.has(part.name)) visit((part.body as NestedSelectorBody).parsed);
    }
  };
  visit(selector);
  return result;
