...
function getImplicitAriaRole(element: Element): string | null {
  ...
  while (ancestor) {
    ...
    if (!parents || !parent || !parents.includes(parent.tagName)) break;
    const parentExplicitRole = getExplicitAriaRole(parent);
    if (
      (parentExplicitRole === 'none' || parentExplicitRole === 'presentation') &&
      !hasPresentationConflictResolution(parent)
    )
...
export function getAriaRole(element: Element): string | null {
  ...
  if (
    ...
    hasPresentationConflictResolution(element)
  )
...
export function isElementHiddenForAria(element: Element, cache: Map<Element, boolean>): boolean {
  if (['STYLE', 'SCRIPT', 'NOSCRIPT', 'TEMPLATE'].includes(element.tagName)) return true;
...
function belongsToDisplayNoneOrAriaHidden(element: Element, cache: Map<Element, boolean>): boolean {
  if (!cache.has(element)) {
    ...
    if (!hidden) {
      ...
      if (parent) hidden = hidden || belongsToDisplayNoneOrAriaHidden(parent, cache);
    }
...
function getPseudoContent(pseudoStyle: CSSStyleDeclaration | undefined) {
  ...
  if (
    ...
    if (display !== 'inline') return ' ' + unquoted + ' ';
    return unquoted;
  }
  return '';
}

export function getElementAccessibleName(
  element: Element,
  ...
  // step 2.
  const accessibleName = normalizeAccessbileName(
    getElementAccessibleNameInternal(element, {
      includeHidden,
      hiddenCache,
...
function getElementAccessibleNameInternal(
  element: Element,
  ...
  const childOptions: AccessibleNameOptions = {
    ...
    embeddedInLabelledBy:
      options.embeddedInLabelledBy === 'self' ? 'descendant' : options.embeddedInLabelledBy,
  ...
  if (
    ...
    options.embeddedInLabelledBy !== 'self' &&
    isElementHiddenForAria(element, options.hiddenCache)
  ) {
  ...
  if (options.embeddedInLabelledBy === 'none') {
    ...
    const accessibleName = refs
      .map((ref) =>
        getElementAccessibleNameInternal(ref, {
          ...options,
  ...
  if (options.embeddedInLabel !== 'none' || options.embeddedInLabelledBy !== 'none') {
    ...
    const isOwnLabelledBy = getIdRefs(element, element.getAttribute('aria-labelledby')).includes(
      element
    );
    if (!isOwnLabel && !isOwnLabelledBy) {
      ...
      if (['combobox', 'listbox'].includes(role)) {
        ...
        return selectedOptions
          .map((option) => getElementAccessibleNameInternal(option, childOptions))
          .join(' ');
      }
  ...
  if (!['presentation', 'none'].includes(role)) {
    ...
    if (element.tagName === 'INPUT' && (element as HTMLInputElement).type === 'image') {
      ...
      if (labels.length) {
        return [...labels]
          .map((label) =>
            getElementAccessibleNameInternal(label, {
              ...options,
  ...
  if (
    ...
    options.embeddedInLabelledBy !== 'none' ||
    options.embeddedInLabel !== 'none' ||
    ...
    for (const owned of getIdRefs(element, element.getAttribute('aria-owns'))) visit(owned);
    tokens.push(getPseudoContent(getComputedStyle(element, '::after')));
    const accessibleName = tokens.join('');
    if (accessibleName.trim()) return accessibleName;
  }
...
export function getAriaSelected(element: Element): boolean {
  ...
  if (element.tagName === 'OPTION') return (element as HTMLOptionElement).selected;
...
export function getAriaChecked(element: Element): boolean | 'mixed' {
  ...
  // https://www.w3.org/TR/html-aam-1.0/#html-attribute-state-and-property-mappings
  if (element.tagName === 'INPUT' && (element as HTMLInputElement).indeterminate) return 'mixed';
  if (
    element.tagName === 'INPUT' &&
...
export function getAriaExpanded(element: Element): boolean {
  ...
  // https://www.w3.org/TR/html-aam-1.0/#html-attribute-state-and-property-mappings
  if (element.tagName === 'DETAILS') return (element as HTMLDetailsElement).open;
