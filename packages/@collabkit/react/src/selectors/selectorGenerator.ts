...
export function generateSelector(
  ...
  try {
    targetElement =
      targetElement.closest('button,select,input,[role=button],[role=checkbox],[role=radio]') ||
      targetElement;
...
function generateSelectorFor(
  ...
  const calculate = (element: Element, allowText: boolean): SelectorToken[] | null => {
    ...
    let textCandidates = allowText
      ? buildTextCandidates(injectedScript, element, element === targetElement).map((token) => [
          token,
        ])
      : [];
    ...
    const checkWithText = (textCandidatesToUse: SelectorToken[][]) => {
      ...
      for (
        ...
        if (
          result &&
          combineScores([...parentTokens, ...bestPossibleInParent]) >= combineScores(result)
        )
          continue;
...
function buildCandidates(injectedScript: InjectedScript, element: Element): SelectorToken[] {
  ...
  for (const attribute of ['data-testid', 'data-test-id', 'data-test']) {
    if (element.getAttribute(attribute))
      candidates.push({
        engine: 'css',
        selector: `[${attribute}=${quoteAttributeValue(element.getAttribute(attribute)!)}]`,
        score: 1,
      });
  }
  ...
  if (element.nodeName === 'INPUT') {
    ...
    if (input.placeholder)
      candidates.push({
        engine: 'css',
        selector: `[placeholder=${quoteAttributeValue(input.placeholder)}]`,
        score: 10,
      });
  }
  if (element.getAttribute('aria-label'))
    candidates.push({
      engine: 'css',
      selector: `[aria-label=${quoteAttributeValue(element.getAttribute('aria-label')!)}]`,
      score: 10,
    });
  if (element.getAttribute('alt') && ['APPLET', 'AREA', 'IMG', 'INPUT'].includes(element.nodeName))
    candidates.push({
      engine: 'css',
      selector: `${cssEscape(element.nodeName.toLowerCase())}[alt=${quoteAttributeValue(
  ...
  if (element.getAttribute('role'))
    candidates.push({
      engine: 'css',
      selector: `${cssEscape(element.nodeName.toLowerCase())}[role=${quoteAttributeValue(
  ...
  if (
    ...
    candidates.push({
      engine: 'css',
      selector: `${cssEscape(element.nodeName.toLowerCase())}[name=${quoteAttributeValue(
        element.getAttribute('name')!
      )}]`,
      score: 50,
    });
  if (
    ...
    if (element.getAttribute('type'))
      candidates.push({
        engine: 'css',
        selector: `${cssEscape(element.nodeName.toLowerCase())}[type=${quoteAttributeValue(
...
function buildTextCandidates(
  ...
  const text = elementText(injectedScript._evaluator._cacheText, element)
    .full.trim()
    .replace(/\s+/g, ' ')
    .substring(0, 80);
  if (!text) return [];
...
function joinTokens(tokens: SelectorToken[]): string {
  ...
  for (const { engine, selector } of tokens) {
    if (
      parts.length &&
      (lastEngine !== 'css' || engine !== 'css' || selector.startsWith(':nth-match('))
    )
      parts.push('>>');
    lastEngine = engine;
