...
import { allEngineNames, parseSelector, stringifySelector } from './selectorParser';
import {
  type TextMatcher,
  elementMatchesText,
...
export class InjectedScript {
  ...
  constructor(
    ...
    if (isUnderTest) (window as any).__injectedScript = this;
  }
  ...
  private _queryLayoutSelector(
    ...
    for (const element of elements) {
      ...
      if (score !== undefined) result.push({ element, score });
    }
  ...
  expectArray(
    ...
    if (received && options.expectedText) {
      ...
      while (mIndex < matchers.length && rIndex < received.length) {
        if (matchers[mIndex].matches(received[rIndex])) ++mIndex;
        ++rIndex;
      }
...
const eventType = new Map<
  string,
  'mouse' | 'keyboard' | 'touch' | 'pointer' | 'focus' | 'drag' | 'wheel'
>([
  ['auxclick', 'mouse'],
  ['click', 'mouse'],
  ['dblclick', 'mouse'],
