...
function validateAttributes(attrs: AttributeSelectorPart[], role: string) {
  for (const attr of attrs) {
    switch (attr.name) {
      ...
      default: {
        throw new Error(
          `Unknown attribute "${attr.name}", must be one of ${kSupportedAttributes
            .map((a) => `"${a}"`)
            .join(', ')}.`
        );
      }
...
export const RoleEngine: SelectorEngine = {
  queryAll(scope: SelectorRoot, selector: string): Element[] {
    ...
    const match = (element: Element) => {
      if (getAriaRole(element) !== role) return;
      ...
      for (const attr of parsed.attributes) {
        ...
        if (!matchesAttributePart(actual, attr)) return;
      }
      if (!includeHidden) {
      ...
      if (nameAttr !== undefined) {
        ...
        if (!matchesAttributePart(accessibleName, nameAttr)) return;
      }
      result.push(element);
    };
    ...
    const query = (root: Element | ShadowRoot | Document) => {
      ...
      if ((root as Element).shadowRoot) shadows.push((root as Element).shadowRoot!);
