...
import { customCSSNames } from './selectorParser';
...
export class SelectorEvaluatorImpl implements SelectorEvaluator {
  ...
  constructor(extraEngines: Map<string, SelectorEngine>) {
    ...
    if (allNames.join('|') !== parserNames.join('|'))
      throw new Error(
        `Please keep customCSSNames in sync with evaluator engines: ${allNames.join(
          '|'
        )} vs ${parserNames.join('|')}`
      );
  }
  ...
  private _cached<T>(cache: QueryCache, main: any, rest: any[], cb: () => T): T {
    ...
    if (entry) return entry.result as T;
    const result = cb();
  ...
  matches(element: Element, s: Selector, context: QueryContext): boolean {
    ...
    try {
      return this._cached<boolean>(
        this._cacheMatches,
        element,
        [selector, context.scope, context.pierceShadow],
        () => {
  ...
  query(context: QueryContext, s: any): Element[] {
    ...
    try {
      return this._cached<Element[]>(
        this._cacheQuery,
        selector,
        [context.scope, context.pierceShadow],
        () => {
  ...
  private _matchesSimple(
    ...
    return this._cached<boolean>(
      ...
      element,
      [simple, context.scope, context.pierceShadow],
      () => {
        const isPossiblyScopeClause = simple.functions.some(
          (f) => f.name === 'scope' || f.name === 'is'
        );
        ...
        for (let i = 0; i < funcs.length; i++) {
          ...
          if (engine.matches !== undefined)
            elements = elements.filter((e) =>
              this._matchesEngine(engine, e, funcs[i].args, context)
            );
        }
        for (let i = 0; i < funcs.length; i++) {
          if (i === firstIndex) continue;
  ...
  private _matchesParents(
    ...
    return this._cached<boolean>(
      ...
      element,
      [complex, index, context.scope, context.pierceShadow],
      () => {
  ...
  private _callMatches(
    ...
    return this._cached<boolean>(
      ...
      element,
      [engine, context.scope, context.pierceShadow, ...args],
      () => {
  ...
  private _callQuery(
    ...
    return this._cached<Element[]>(
      ...
      engine,
      [context.scope, context.pierceShadow, ...args],
      () => {
    ...
    return this._cached<Element[]>(
      ...
      () => {
        ...
        function query(root: Element | ShadowRoot | Document) {
          ...
          if ((root as Element).shadowRoot) query((root as Element).shadowRoot!);
...
const scopeEngine: SelectorEngine = {
  ...
  query(
    ...
    if (context.scope.nodeType === 1 /* Node.ELEMENT_NODE */) return [context.scope as Element];
    return [];
  },
};
