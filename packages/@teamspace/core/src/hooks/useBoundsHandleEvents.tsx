...
import { TLBoundsEdge, TLBoundsCorner, TLBoundsHandleTarget } from '~types'
...
export function useBoundsHandleEvents(
  ...
  const target: TLBoundsHandleTarget = { type: 'boundsHandle', id }
  ...
  const onPointerMove = React.useCallback(
    (e: React.PointerEvent) => {
      ...
      if (e.currentTarget.hasPointerCapture(e.pointerId)) {
        callbacks.onDragBoundsHandle?.(inputs.pointerMove(e, target), e)
      }
  ...
  const onPointerEnter = React.useCallback(
    (e: React.PointerEvent) => {
      if (!inputs.pointerIsValid(e)) return
      callbacks.onHoverBoundsHandle?.(inputs.pointerEnter(e, target), e)
    },
    [inputs, callbacks, id]
  )
  ...
  const onPointerLeave = React.useCallback(
    (e: React.PointerEvent) => {
      if (!inputs.pointerIsValid(e)) return
      callbacks.onUnhoverBoundsHandle?.(inputs.pointerEnter(e, target), e)
    },
    [inputs, callbacks, id]
  )
