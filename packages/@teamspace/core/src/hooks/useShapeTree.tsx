...
function addToShapeTree<T extends TLShape, M extends Record<string, unknown>>(
  ...
  const isSelected = pageState.selectedIds.includes(shape.id)

  const node: IShapeTreeNode<T, M> = {
    shape,
    meta: meta as any,
    hasSize: !!pageState.hasSizes[shape.id],
    isEditing: pageState.editingId === shape.id,
    isBinding: pageState.bindingTargetId === shape.id,
    isSelected,
    isDragging: isSelected && pageState.isDragging,
    isResizing: isSelected && pageState.isResizing,
    isHovered:
...
export function useShapeTree<T extends TLShape, M extends Record<string, unknown>>(
  ...
  Object.values(page.shapes)
    ...
    .forEach((shape) => {
      ...
      if (pageState.hiddenIds.includes(shape.id)) {
        return
      }
