...
export const startTranslatingTableColumn = (state: State, info: TLPointerInfo<TLTarget>) => {
  const target = state.store.editing.pointingId;
  if (target?.type !== 'tableColumn') {
    console.warn('[startTranslatingTableColumn]: no column selected');
    return;
  }
